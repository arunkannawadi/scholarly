...
class ProxyGenerator(object):
    ...
    def _use_proxy(self, http: str, https: str = None) -> bool:
        ...
        if self.proxy_mode == ProxyMode.SCRAPERAPI:
            r = requests.get("http://api.scraperapi.com/account", params={'api_key': self._API_KEY}, timeout=60).json(timeout=60)
            if "error" in r:
    ...
    def ScraperAPI(self, API_KEY, country_code=None, premium=False, render=False):
        ...
        r = requests.get("http://api.scraperapi.com/account", params={'api_key': API_KEY}, timeout=60).json(timeout=60)
        if "error" in r:
            self.logger.warning(r["error"])
            return False
